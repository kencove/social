...
class SocialNetworkPost(models.Model):
    ...
    account_ids = fields.Many2many(
        "social.network.account", required=True, ondelete="restrict"
    )
    active = fields.Boolean(default=True)
    message = fields.Text(required=True)
    campaign_id = fields.Many2one("utm.campaign")
    send_post = fields.Selection(
        [("now", "Now"), ("schedule", "Schedule")], required=True, default="now"
    )
    ...
    count_post_impression = fields.Integer(
        compute="_compute_post_statistics", default=0
    )
    ...
    def default_get(self, fields):
        ...
        account_ids = (
            self.env["social.network.account"].with_company(self.env.company).search([])
        )
        res["account_ids"] = [(6, 0, account_ids.ids)]
        return res
    ...
    def _run_send_post(self):
        post_accounts = self.env["social.network.post"].search(
            [("state", "=", "planned"), ("send_post", "=", "schedule")]
        )
