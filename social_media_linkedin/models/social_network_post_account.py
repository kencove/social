...
class SocialNetworkPostAccount(models.Model):
    ...
    def _action_post(self):
        ...
        failed_post_account = self.env["social.network.post.account"]
        for post_account in post_accounts:
            post_entity = post_account.account_id.create_restclient_linkedin(
                ...
                video_ids=post_account.video_ids,
            )
            if post_entity:
                post_account.write(
                    {
    ...
    def action_like_post(self, author_urn=None):
        super().action_like_post(author_urn)
    ...
    def get_comments(self):
        ...
        if "linkedin" == self.account_id.media_type and self.linkedin_post_account_urn:
            ...
            if response.status_code == 200:
                ...
                raise ValidationError(f"Error GET COMMENTS LINKEDIN: {response.json()}")
        return list(itertools.chain(data, comments))

    def create_linkedin_comment(self, comment, image_base64):
        if "linkedin" == self.account_id.media_type:
            ...
            if image_base64:
                asset_response = self.account_id._prepare_images_for_post(
                    image_datas=image_base64
                )
                if len(asset_response) > 0:
                    if image_base64:
                        json_data.update(
                            {
                                "content": [
                                    {"entity": {"digitalmediaAsset": asset_response[0]}}
                                ]
                            }
                        )
            response = self.account_id._request_linkedin(
            ...
            if response.status_code != 201:
                _logger.error(
                    "ERROR CREATE COMMENT LINKEDIN: {}".format(
